...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'IUPHAR/BPS Guide to PHARMACOLOGY');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        ligand: { type: 'string', description: 'Drug/ligand name, e.g. "imatinib". Resolved against the GtoPdb ligand list.' },
        ligand_id: { type: ['number', 'string'], description: 'GtoPdb ligand id, e.g. 4139 for aspirin. Optional — give this OR ligand.' },
        limit: { type: 'number', description: 'Max interactions to return (default 25).' },
      },
      required: [],
    },
...
/**
 * ligand_id is a GtoPdb-internal integer. No caller knows one, so a model
 * supplies it from memory — and when it is wrong the tool answered confidently
 * about a different substance: "what proteins does imatinib target?" arrived as
...
async function resolveLigand(args: Record<string, unknown>): Promise<{ id: string; name: string | null; requestedName?: string }> {
  ...
  // Prefer an exact, approved match over a substring hit.
  const exact = list.filter((l) => String(l.name ?? '').toLowerCase() === name.toLowerCase());
  const pick = exact.find((l) => l.approved === true) ?? exact[0] ?? list[0];
  return { id: String(pick.ligandId), name: typeof pick.name === 'string' ? pick.name : null, requestedName: name };
}
...
async function targetNames(ids: string[]): Promise<Map<string, string>> {
  ...
  await Promise.all(ids.slice(0, 40).map(async (id) => {
    try {
      ...
      // A missing target name must not cost the interaction row.
    }
  }));
  return out;
}
  ...
  try {
    switch (name) {
      ...
      case 'ligand_interactions': {
        ...
        return {
          ligand_id: resolved.id,
          // Always say WHICH compound was read. A caller who passed a wrong id
          // previously got `{ligand_id: 5984, count: 0}` and read it as "this
          // drug has no known targets" — 5984 is herbimycin A, not imatinib.
          ligand: resolved.name,
          ...(resolved.requestedName ? { resolved_from: resolved.requestedName } : {}),
          count: list.length,
          interactions,
          ...(list.length === 0 ? {
            found: false,
            reason: 'no_curated_interactions',
            hint: `The Guide to PHARMACOLOGY holds no curated interactions for ligand ${resolved.id}${resolved.name ? ` (${resolved.name})` : ''}. Confirm this is the compound you meant — ids are GtoPdb-internal and a wrong one resolves to a different substance. Search by name with search_ligands, and note GtoPdb curates selected pharmacology rather than every measured binding, so a real drug can legitimately have none.`,
          } : {}),
        };
      }
      default:
